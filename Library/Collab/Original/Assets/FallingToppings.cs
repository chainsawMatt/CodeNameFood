    ...
    void Update()
    {
        ...
        else
        {
            if (CaughtTops != null)
            {
                /* 
                Stack the toppings
                Smooth time = Time it takes for game object to reposition itself in the stack. 
                0 = Static stack - Toppings are locked with movements of player. Still fun to play!
                0.035 is where we started. Looks pretty zany!
                
                 */
                x = Mathf.SmoothDamp(transform.position.x, CaughtTops.transform.position.x, ref z, 0.0f);
                y = Mathf.SmoothDamp(transform.position.y, CaughtTops.transform.position.y + 1.5f, ref z, 0.0f);
